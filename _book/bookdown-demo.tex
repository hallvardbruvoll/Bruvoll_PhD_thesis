...
For the following tests, all images were constructed in R as data tables with x and y coordinates for each element (``house''), rendered with the \emph{ggplot2} package (Wickham, 2016). More specifically, houses were represented as a geom\_tile() layer with black fill and given height and width, and the background was set to white, while the theme\_void() function allowed for removing the axes and grids altogether. With this procedure it was possible to generate incremental change in a single variable on otherwise identical images. However, as some of the variables are inherently connected, full separation was not always possible. All the generated and analysed images are included in \#Make Appendix, and the original files, as well as the code for generating them, are available in the \#online repository.

\hypertarget{effects-from-image-size-and-element-size-and-element-count}{%
\section{Effects from image size and element size and element count}\label{effects-from-image-size-and-element-size-and-element-count}}
...
\caption{Different image sizes and house sizes}\label{fig:08-IS-HS}
\end{figure}
...
\caption{Element count (N) and image size.}\label{fig:08-N-IS}
\end{figure}
...
\caption{Variable house count (N) and house size, fixed image size and density}\label{fig:08-N-HS}
\end{figure}
